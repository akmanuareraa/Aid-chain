  ...
  return (
    <>
      <div className="flex flex-col items-center mt-10">
        <p className="font-bold py-6">Register a Distributor</p>
        <form className="grid grid-cols-3 gap-4 mb-4 items-center">
          ...
          />
          <label className="col-start-1 col-span-1 text-gray-700">Area:</label>
          <input
            type="text"
            placeholder="Enter Area of Operation"
            className="col-start-2 col-span-2 px-3 py-2 border border-gray-400 rounded-lg  focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent input input-bordered w-full max-w-xs h-8"
            onInputCapture={(e) => handleInput("area", e)}
            value={formData.area}
          />
          <label className="col-start-1 col-span-1 text-gray-700">
            Distributor Type:
          ...
          <label className="inline-flex items-center space-x-4">
            ...
            <div className="flex flex-row items-center">
              <input
                ...
                name="army"
                value="army"
                checked={formData.type === "army"}
                onChange={handleTypeChange}
                className="form-radio h-4 w-4 text-blue-600"
              />
              <span className="ml-2 text-gray-700">Army</span>
            </div>
          </label>
        </form>
        ...
        <button
          ...
          onClick={async () => {
            if (props.appState.account === "") {
              ...
              await props.registerDistributor(
                formData.name,
                formData.username,
                formData.password,
                formData.type,
